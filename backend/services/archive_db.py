...
def _ensure_jobs_priority(conn: sqlite3.Connection) -> None:
    ...
    if "priority" in cols:
        # Already migrated (fresh DBs get priority from SCHEMA directly):
        # still guarantee the queue index exists — SCHEMA must not create it
        # (it would crash on pre-migration DBs before this rebuild runs).
        conn.execute(
