...
-- Worker liveness: transcribe/backfill workers ping this every poll
-- iteration; the search router uses worker_live() to decide whether
-- enrichment entries are honest (a live worker means jobs actually run).
...
# --- worker heartbeat / FTS maintenance -------------------------------------
# Workers (transcribe, chat backfill) ping worker_heartbeats every poll
# iteration; the search router calls worker_live() to decide whether
# transcribe enrichment entries are honest (a live worker means queued jobs
# will actually run, so presenting "transcribing…" is truthful).

def worker_heartbeat(tag: str) -> None:
    """Record that the named worker is alive, right now (upsert)."""
    execute(
...
def worker_live(age_s: int = 30) -> bool:
    """True when the transcribe worker beat within the last age_s seconds.

    Absent or unparseable heartbeat -> False (enrichment must not claim a
    worker that is not demonstrably alive)."""
    rows = query("SELECT at FROM worker_heartbeats WHERE tag = 'transcribe'")
    if not rows:
        return False
    from datetime import datetime, timezone
    ...
    try:
        ...
        return False
    return (datetime.now(timezone.utc) - at).total_seconds() <= age_s


def optimize_fts() -> None:
    """Merge FTS5 b-tree segments for both indexes — run after heavy batch
    writes (a finished transcribe job, a completed chat backfill) so search
    stays fast. Best-effort: a busy/locked index just logs and moves on."""
