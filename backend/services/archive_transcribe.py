...
def _claim_next_job() -> Optional[dict]:
    ...
    for row in rows:
        if row["status"] == "queued":
            ...
            # Stale-reclaim CAS: the UPDATE re-checks the stale-window
            # condition the SELECT used, so two workers that both read the
            # same stale 'running' row cannot both claim it — the first
            # claim refreshes updated_at and the second's WHERE matches
            # zero rows (rowcount 0 -> skip). Without the condition both
            # UPDATEs would hit (status stays 'running' either way) and two
            # workers would transcribe the same video.
            cutoff = chat_cutoff if row["kind"] == "chat" else transcribe_cutoff
            cur = archive_db.execute(
                "UPDATE archive_jobs SET status = 'running', updated_at = ? "
