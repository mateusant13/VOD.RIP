...
// ----------------------------------------------
// VOD.RIP bridge — token display + pairing state
// ----------------------------------------------

const bridgeStateEl = document.querySelector('#bridgeState');
const bridgeTokenEl = document.querySelector('#bridgeToken');
const bridgeDetailEl = document.querySelector('#bridgeDetail');
const copyTokenBtn = document.querySelector('#copyToken');
...
const renderBridge = async () => {
  ...
  try {
    ...
    // storage unavailable — leave token empty
  }
  bridgeTokenEl.textContent = token
    ? `${token.slice(0, 8)}…${token.slice(-4)}`
    : 'unavailable';
  bridgeTokenEl.dataset.token = token;
  copyTokenBtn.disabled = !token;

  try {
    const res = await fetch(`${await getApiBase()}/api/session/cookies/status`);
    if (!res.ok) throw new Error(`status ${res.status}`);
    const status = await res.json();
    const counts = Object.entries(status.platforms || {})
      .map(([p, n]) => `${p}: ${n}`)
      .join(', ');
    bridgeStateEl.textContent = status.paired ? 'paired' : 'pending';
    bridgeStateEl.classList.toggle('paired', Boolean(status.paired));
    bridgeDetailEl.textContent = status.paired
      ? `paired — stored ${counts || 'no cookies yet'}`
      : 'pending — first sync will pair this install with the VOD.RIP app';
  } catch {
    bridgeStateEl.textContent = 'offline';
    bridgeStateEl.classList.remove('paired');
    bridgeDetailEl.textContent = 'VOD.RIP app not reachable at 127.0.0.1:7897';
  }
};

copyTokenBtn.addEventListener('click', async () => {
  const token = bridgeTokenEl.dataset.token;
  if (!token) return;
  try {
    await navigator.clipboard.writeText(token);
  } catch {
    // fallback for browsers without async clipboard in extension popups
